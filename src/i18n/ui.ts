export const languages = {
  en: 'English',
  fr: 'Français',
};

export const defaultLang = 'en';

export const ui = {
  en: {
    // Navigation
    'nav.delight': 'Delight',
    'nav.learning': 'Learning (music)',
    'nav.discovery': 'Discovery',
    'nav.sharing': 'Sharing',
    'nav.about': 'About',
    'nav.home': 'HOME',

    // Sub-navigation: Delight
    'nav.labande': 'La Bande<br/>La Loingtaine',
    'nav.concerts': 'Concerts',
    'nav.upcoming': 'Upcoming<br/>concerts',
    'nav.past': 'Past<br/>concerts',
    'nav.schubertiades': 'Association<br/>Schubertiades',
    'nav.gallery': 'Photo gallery',

    // Sub-navigation: Learning
    'nav.academies': 'Academies',
    'nav.masterclasses': 'Masterclasses',
    'nav.workshops': 'Workshops',
    'nav.test': 'Test',

    // Sub-navigation: Discovery
    'nav.science': 'Science, Culture and Nature',
    'nav.forum': 'Ideas Forum',

    // Sub-navigation: Sharing
    'nav.books': 'Books',
    'nav.publishing': 'La Loingtaine Publishing',
    'nav.library': 'Virtual Library Project',
    'nav.echoes': 'Festival Echoes',
    'nav.conferences': 'Conferences',

    // About
    'nav.laloingtaine': 'La Loingtaine',
    'nav.whynew': 'Why this new site?',
    'nav.whoarewe': 'Who are we?',
    'nav.calendar': 'Calendar',
    'nav.howto': 'How to get there?',

    // Footer
    'footer.address': '40 rue Grande, Sorques, 77690 Montigny sur Loing.',
    'footer.reservation': 'Réservation: 01 64 45 83 14 / 06 22 85 11 86.',
    'footer.copyright': 'Copyright © 2024. La loingtaine. All rights reserved.',

    // Site
    'site.tagline': 'Au plus près de la culture et de la musique',
    'site.title': 'La Loingtaine',
  },
  fr: {
    // Navigation
    'nav.delight': 'Plaisir',
    'nav.learning': 'Transmission',
    'nav.discovery': 'Découverte',
    'nav.sharing': 'Partage',
    'nav.about': 'À Propos',
    'nav.home': 'ACCUEIL',

    // Sub-navigation: Delight
    'nav.labande': 'La Bande<br/>La Loingtaine',
    'nav.concerts': 'Concerts',
    'nav.upcoming': 'Prochains<br/>concerts',
    'nav.past': 'Concerts<br/>passés',
    'nav.schubertiades': 'Association<br/>Schubertiades',
    'nav.gallery': 'Galerie photos',

    // Sub-navigation: Learning
    'nav.academies': 'Académies',
    'nav.masterclasses': 'Masterclasses',
    'nav.workshops': 'Ateliers',
    'nav.test': 'Teste',

    // Sub-navigation: Discovery
    'nav.science': 'Science, Culture et Nature',
    'nav.forum': 'Forum des Idées',

    // Sub-navigation: Sharing
    'nav.books': 'Livres',
    'nav.publishing': 'Éditions La Loingtaine',
    'nav.library': 'Bibliothèque Virtuelle',
    'nav.echoes': 'Échos du Festival',
    'nav.conferences': 'Conférences',

    // About
    'nav.laloingtaine': 'La Loingtaine',
    'nav.whynew': 'Pourquoi ce nouveau site?',
    'nav.whoarewe': 'Qui sommes-nous?',
    'nav.calendar': 'Calendrier',
    'nav.howto': 'Comment y aller?',

    // Footer
    'footer.address': '40 rue Grande, Sorques, 77690 Montigny sur Loing.',
    'footer.reservation': 'Réservation: 01 64 45 83 14 / 06 22 85 11 86.',
    'footer.copyright': 'Copyright © 2024. La loingtaine. Tous droits réservés.',

    // Site
    'site.tagline': 'Au plus près de la culture et de la musique',
    'site.title': 'La Loingtaine',
  },
} as const;
